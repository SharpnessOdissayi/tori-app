...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      // z-[60] so the overlay covers the global AccessibilityFab (z-55)
      // and ThemeToggleFab (z-56). Otherwise those two buttons punched
      // through the dialog backdrop and sat on top of dialog content on
      // mobile, hiding submit buttons and checkboxes behind them.
...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        // z-[70] keeps dialog content above the overlay (z-60) and the
        // global FABs (z-55/56), so interactive rows + submit bars are
        // never hidden behind the floating accessibility/theme buttons.
